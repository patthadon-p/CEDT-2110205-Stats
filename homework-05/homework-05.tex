...
\begin{subproblems_alpha}
	...
	\item The integral of a valid PDF, \( \displaystyle \int_{-\infty}^{\infty} f(x)\,dx \), over its entire range must equal 1.
...
\begin{solution}
	...
	\begin{align*}
		...
		F(x) & = \begin{cases}
			         0                                                                   & x < 0            \\
			         \int_{0}^{x} \frac{1}{4}t\,dt                                       & 0 \leq x < 2     \\
			         \int_{0}^{2} \frac{1}{4}t\,dt + \int_{2}^{x} \frac{1}{4}(4 - t)\,dt & 2 \leq x < 4     \\
			         1                                                                   & \text{otherwise}
		         \end{cases} \\
	\end{align*}
	...
	\[
		F(x) = \begin{cases}
			...
			\paren{\frac{x^2}{8}}_{x=2} + \paren{x - \frac{x^2}{8} - \frac{3}{2}} & 2 \leq x < 4     \\
			1                                                                     & \text{otherwise}
		\end{cases}
	\]
	\[
...
\begin{solution}
	...
	Thus, we can denote this as: \( \boxed{ T \sim \text{Uniform}(0, 15) } \)
...
\begin{tosubmit}
	...
	\par The exponential distribution has the memoryless property, which means that the probability of surviving an additional time period is independent of how long it has already survived.
	\par Thus, we need to find \( P(X > 7 \mid X > 3) \), which is equivalent to \( P(X > 4) \) due to the memoryless property.
...
\begin{solution}
	From Problem 8, we know that the lifespan of the hard drive follows an exponential distribution with parameter \( \lambda = \frac{1}{4} \).
	\par The CDF of an exponential distribution is given by:
...
\begin{tosubmit}
	...
	\par\textbf{a).} To find the probability that a randomly selected student scores between 65 and 85, we need to calculate \( P(65 < X < 85) \).
	...
	\par\textbf{b).} For a continuous random variable, the probability of scoring exactly any specific value is 0.
...
% ================================================================================ %
\begin{problem}
A bakery sells two types of muffins: blueberry \( B \sim \mathcal{N}(120, 16) \) and chocolate chip \( C \sim \mathcal{N}(125, 9) \) (in grams).
You buy one of each. What is the probability that the blueberry muffin is heavier than the chocolate chip muffin?
(Hint: Consider the distribution of the difference \( D = B - C \)).
\end{problem}
...
\begin{solution}
	...
	\[ P(X \geq 30) = 1 - P(Z < 0.92) \approx 1 - 0.8212 = \boxed{0.1788} \]
\end{solution}
% ================================================================================ %
