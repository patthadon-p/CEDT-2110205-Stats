...
\begin{subproblems_alpha}
	\item What is the expectation \( \text{E}[X] \)?
	\item What is the variance \( \text{Var}(X) \)?
	\item Let \( Y = 5X - 3 \). Calculate \( \text{E}[Y] \) and \( \text{Var}(Y) \).
...
\begin{solution}
	...
	\[
		\text{E}[Z] = \frac{n + 1}{2}, \quad \text{Var}(Z) = \frac{n^2 - 1}{12}
	\]
	Therefore,
	\[
		\text{E}[X] = \frac{10 + 1}{2} = \frac{11}{2} = \boxed{5.5} \; \textbf{a).}
	\]
	and,
	\[
...
\begin{subproblems_alpha}
	\item Find the expectation \( \text{E}[X] \).
	\item Find the variance \( \text{Var}(X) \).
	\item Calculate \( \text{E}[X^2] \).
\end{subproblems_alpha}
\end{problem}
...
\begin{solution}
	...
	Thus,
	\[
		\text{E}[X] = \boxed{0.8} \; \textbf{a).}
	\]
	and,
	\[
		\text{Var}(X) = 0.8(1 - 0.8) = 0.8 \cdot 0.2 = \boxed{0.16} \; \textbf{b).}
	\]
	Because, \( \text{Var}(X) = \text{E}[X^2] - \text{E}[X]^2 \), so that:
	\begin{align*}
...
\begin{subproblems_alpha}
	...
	\item What is the expected number of defective chips, \( \text{E}[Y] \)?
	\item What is the variance of the number of defective chips, \( \text{Var}(Y) \)?
\end{subproblems_alpha}
\end{problem}

\begin{tosubmit}
	According to the statement, \( Y \sim \text{Binomial}(20, 0.05) \)

	\par\textbf{a).} The probability of finding exactly 2 defective chips
	can be calculated using the binomial probability formula:
	\[
		P(Y = 2) = \binom{20}{2} (0.05)^{2} (0.95)^{18} \approx \boxed{0.1887}
	\]

	\par\textbf{b).} The expected number of defective chips, \( \text{E}[Y] \),
	is the expected value of a binomial distribution, \( Y \sim \text{Binomial}(20, 0.05) \):
	\[
		\text{E}[Y] = (20)(0.05) = \boxed{1}
	\]

	\par\textbf{c).} The variance of number of defective chips, \( \text{Var}[Y] \),
	is the variance of a binomial distribution, \( Y \sim \text{Binomial}(20, 0.05) \):
	\[
		\text{Var}(Y) = (20)(0.05)(1-0.05) = \boxed{0.95}
	\]
\end{tosubmit}
% ================================================================================ %
...
\begin{subproblems_alpha}
	...
	\item What is the expectation \( \text{E}[X] \) and variance \( \text{Var}(X) \).
...
\begin{solution}
	...
	Thus,
	\[
		\boxed{\text{E}[X] = \text{Var}(X) = 8} \; \textbf{b).}
	\]

	The expected value of the total number of calls in one hour is \( \text{E}[X] = 8 \), so that
	\[
...
% ================================================================================ %
\begin{problem}
Let X be a random variable with \( \text{E}[X] = 6 \) and \( \text{Var}(X) = 2 \).
Let \( Y = -2X + 7 \).
\begin{subproblems_alpha}
	\item Find \( \text{E}[Y] \).
	\item Find \( \text{Var}(Y) \).
...
\begin{solution}
	Because \( Y = -2X + 7 \):
	\[
		\text{E}[Y] = -2\text{E}[X] + 7 = -2(6) + 7 = -12 + 7 = \boxed{-5} \; \textbf{a).}
	\]

	and,
	\[
		\text{Var}(Y) = (-2)^2 \text{Var}(X) = 4(2) = \boxed{8} \; \textbf{b).}
	\]

	\myspace

	From the relation, \( \text{Var}(X) = \text{E}[X^2] - \text{E}[X]^2 \), we can compute \( \text{E}[X^2] \):
	\begin{align*}
		\text{E}[X^2] & = \text{Var}(X) + \text{E}[X]^2 \\
		              & = 2 + 6^2                       \\
		              & = 2 + 36                        \\
...
\begin{subproblems_alpha}
	\item Find the expected value of the prize, \( \text{E}[P] \).
...
\begin{tosubmit}
	\[
		\text{E}[P] = \text{E}[3X^2 - 5] = 3\text{E}[X^2] - 5
	\]

	\par While \( X \sim \text{Uniform}(12) \), the expectation of Uniform Distribution is:
	\[
		\text{E}[X] = \frac{n+1}{2} = \frac{12+1}{2} = 6.5
	\]
	...
	We can calculate \( \text{E}[X^2] \) as follows:
	\begin{align*}
		\text{Var}(X) & = \text{E}[X^2] - \paren{\text{E}[x]}^2 \\
		\text{E}[X^2] & = \text{Var}(X) + \paren{\text{E}[x]}^2 \\
		              & = \frac{143}{12} + (6.5)^2              \\
		              & = \frac{143}{12} + \frac{169}{4}        \\
		              & = \frac{650}{12}                        \\
		\text{E}[X^2] & = \frac{325}{6}
	\end{align*}

	we know that:
	\begin{align*}
		\text{E}[P] & = 3\text{E}[X^2] - 5          \\
		            & = 3 \times \frac{325}{6} - 5  \\
		            & = \frac{325}{2} - 5           \\
		            & = \frac{315}{2}               \\
		            & = \boxed{157.5} \; \text{a).}
...
\begin{subproblems_alpha}
	\item What is the expected total cost, \( \text{E}[100N] \)?
...
\begin{solution}
	...
	\[
		\text{E}[N] = \frac{1}{p} = \frac{1}{0.4} = 2.5
	\]
	Thus, the expected total cost is:
	\[
		\text{E}[100N] = 100\text{E}[N] = 100 \times 2.5 = \boxed{250} \; \textbf{a).}
	\]
...
\begin{solution}
	...
	\begin{align*}
		P(\text{red}) & = P(\text{red} | A) P(A) + P(\text{red} | B) P(B)       \\
		              & = \paren{\frac{3}{10}}(0.5) + \paren{\frac{6}{10}}(0.5) \\
		              & = \frac{3}{20} + \frac{6}{20} = \frac{9}{20}
	\end{align*}

	Substituting these values back into Bayes' theorem:
	\[
		P(A | \text{red}) = \frac{\paren{\frac{3}{10}}(0.5)}{\frac{9}{20}} = \frac{\frac{3}{20}}{\frac{9}{20}} = \frac{3}{9} = \frac{1}{3}
	\]
...
\begin{subproblems_alpha}
	\item Find the expected number of heads, \( \text{E}[H] \).
...
\begin{solution}
	...
	\[
		\text{Var}(H) = \text{E}[\text{Var}(H | K)] + \text{Var}(\text{E}[H | K])
	\]
	Thus,
	\begin{align*}
		\text{Var}(H) & = \text{E}[\text{Var}(H | K)] + \text{Var}(\text{E}[H | K])                    \\
		              & = \text{E}[K \times 0.7 \times (1 - 0.7)] + \text{Var}(K \times 0.7)           \\
		              & = \text{E}[K \times 0.21] + \paren{(0.7)^2 \times \text{Var}(K)}               \\
		              & = \paren{0.21 \times \text{E}[K]} + \paren{0.49 \times \text{Var}(K)}          \\
