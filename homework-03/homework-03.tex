...
\begin{tosubmit}
	\par The probability of ``Hopeful'' winning the 4th race is the same with the probability of winning--\(\boxed{0.05}\). \textbf{a).}
	\par The probability that his first win occurs after the 2nd race is
...
\begin{solution}
	\par\textbf{a).} \, The probability distribution for \( X \) is a \( \boxed{\textbf{Bernoulli distribution}} \).
	Because \( X \) can take on only two possible outcomes: winning (1) or losing (0).

	\myspace

	\par Additionally, the probability of winning is \( p = \frac{1}{8} \) (since all horses have an equal chance), and the probability of losing is \( 1 - p = \frac{7}{8} \).
	\par Thus, \( \boxed{ X \sim \text{Bernoulli}\paren{\frac{1}{8}} } \).

	\myspace

	\par\textbf{b).} \, The probability mass function (PMF) of \( X \) is given by:
	\[\boxed{
			P(X = x) =
			\begin{cases}
				\frac{1}{8}, & x=1               \\
